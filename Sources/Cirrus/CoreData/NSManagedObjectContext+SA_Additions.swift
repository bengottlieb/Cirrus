...
public extension NSManagedObjectContext {
	...
	@discardableResult
	func saveContext(toDisk: Bool = false, ignoreHasChanges: Bool = false) async -> Error? {
		await withCheckedContinuation { continuation in
			self.saveContext(wait: true, toDisk: toDisk, ignoreHasChanges: ignoreHasChanges) { error in
